 ...
 *
 * 2. 从Kafka中消费数据 (ODS layer: file->flume->kafka 統一 topic)
 *
 * 3. 处理数据
 *     3.1 转换数据结构 JSON -> Object
 *           专用结构： 單獨封裝自定義Bean object
 *           通用结构:  Map JsonObject
 *     3.2 分流
 *
 * 4. 写出到DWD层(啟動, 頁面訪問, 動作, 曝光, 錯誤日誌）
 */
...
//    val DWD_ERROR_LOG_TOPIC : String = "DWD_ERROR_LOG_TOPIC_1018" // 错误数据
    ...
    //              } else {
    //                // 非错误数据, 提取頁面, 啟動數據中的公共字段
    //                val commonObj: JSONObject = jsonObj.getJSONObject("common")
    //                val ar: String = commonObj.getString("ar")
    //                val uid: String = commonObj.getString("uid")
    //                val os: String = commonObj.getString("os")
    //                val ch: String = commonObj.getString("ch") //channel
    //                val isNew: String = commonObj.getString("is_new")
    //                val md: String = commonObj.getString("md") //model
    //                val mid: String = commonObj.getString("mid")
    //                val vc: String = commonObj.getString("vc") //版本
    //                val ba: String = commonObj.getString("ba") //廠牌
    //                //提取时间戳
    ...
    //         */
    //
    //        //foreachRDD里面，foreach外面: 提交offset??? B  Driver段执行，一批次执行一次（周期性）
    //        MyOffsetsUtils.saveOffset(topicName,groupId,offsetRanges)
    //        //foreachRDD里面，foreach外面: 刷写Kafka缓冲区??? B  Driver段执行，一批次执行一次（周期性） 分流是在executor端完成，driver端做刷写，刷的不是同一个对象的缓冲区.
    //      }
    //    )
